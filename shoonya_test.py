...
# ==============================================================================
st.set_page_config(page_title="QuantScalper AI v35.0 [ULTIMATE]", layout="wide", initial_sidebar_state="collapsed")
...
with col_h1: 
    ...
    st.markdown(f"<h1 style='margin:0; font-weight:800;'>QUANT<span style='color:#deff9a;'>SCALPER AI</span> v35.0 {ws_badge}</h1>", unsafe_allow_html=True)
with col_h2: 
    st.markdown(f"<div style='text-align:right; font-size:18px; font-weight:bold;'>Total Trades: {total_trades} | Day PnL: <span style='color:{pnl_color};'>{net_pnl} pts</span></div>", unsafe_allow_html=True)
...
with c_opt3: trade_qty = st.number_input(f"Qty (Auto Lot: {get_lot_size(index_choice)})", min_value=get_lot_size(index_choice), step=get_lot_size(index_choice), value=get_lot_size(index_choice))
...
with st.spinner('Syncing Macro Trend...'):
    ...
    def calculate_trailing_sl(entry, current, pnl_points, trade_type):
        if pnl_points >= 10: return entry # Cost-to-cost if 10 pts profit
        if trade_type == 'CE': return entry - safe_sl_pts
        else: return entry + safe_sl_pts
    ...
    with c1:
        if st.button(f"🟢 BUY CE ({atm_strike})", disabled=not can_trade or st.session_state.trade_active, use_container_width=True):
            if live_mode and st.session_state.shoonya_token:
                ...
                if succ: st.session_state.trade_active = True; st.session_state.trade_details = {'Type':'CE', 'Sym':ce_symbol, 'Qty':trade_qty, 'Status':'LIVE', 'Entry':curr_p, 'Time':datetime.datetime.now().strftime("%H:%M:%S")}
            else:
                st.session_state.trade_active = True; st.session_state.trade_details = {'Type':'CE', 'Sym':ce_symbol, 'Qty':trade_qty, 'Status':'PAPER', 'Entry':curr_p, 'Time':datetime.datetime.now().strftime("%H:%M:%S")}
            st.rerun()
    ...
    with c2:
        if st.button(f"🔴 BUY PE ({atm_strike})", disabled=not can_trade or st.session_state.trade_active, use_container_width=True):
            if live_mode and st.session_state.shoonya_token:
                ...
                if succ: st.session_state.trade_active = True; st.session_state.trade_details = {'Type':'PE', 'Sym':pe_symbol, 'Qty':trade_qty, 'Status':'LIVE', 'Entry':curr_p, 'Time':datetime.datetime.now().strftime("%H:%M:%S")}
            else:
                st.session_state.trade_active = True; st.session_state.trade_details = {'Type':'PE', 'Sym':pe_symbol, 'Qty':trade_qty, 'Status':'PAPER', 'Entry':curr_p, 'Time':datetime.datetime.now().strftime("%H:%M:%S")}
            st.rerun()
    ...
    if st.session_state.trade_active:
        with c3:
            ...
            trail_sl = calculate_trailing_sl(t['Entry'], curr_p, live_points, t['Type'])
            pcol = "#00ff66" if live_points >= 0 else "#ff3333"
            
            st.markdown(f"""
            <div class='live-pnl-box'>
                <div style='display:flex; justify-content:space-between;'>
                    <div><b>{t['Status']} {t['Type']}</b> | {t['Sym']} <br> <span style='color:#8b949e;'>Entry: ₹{t['Entry']} | Trail SL: ₹{trail_sl}</span></div>
                    <div style='text-align:right;'><span style='color:#8b949e;'>Live PnL</span><br><b style='color:{pcol}; font-size:22px;'>{'+' if live_points>0 else ''}{live_points} pts</b></div>
                </div>
            </div>""", unsafe_allow_html=True)
            
            if st.button("⏹️ SQUARE-OFF & SAVE LOG", use_container_width=True):
                if t['Status'] == 'LIVE' and st.session_state.shoonya_token: place_shoonya_order(st.session_state.shoonya_token, t['Sym'], t['Qty'], 'S')
                st.session_state.trade_history.append({"Time": t['Time'], "Type": t['Type'], "Strike": t['Sym'], "Entry": t['Entry'], "Exit": curr_p, "PnL (Points)": live_points})
    ...
    if df is not None and not df.empty:
        ...
        if 'VWAP' in plot_df.columns: fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df['VWAP'], name='VWAP', line=dict(color='#00ffff', width=2, dash='dash')))
        fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df['EMA_200'], name='200 EMA', line=dict(color='#ffaa00', width=2)))
        if st.session_state.trade_active: fig.add_hline(y=st.session_state.trade_details['Entry'], line_dash="dot", line_color="#00ff66", annotation_text="Entry")
        fig.update_layout(template='plotly_dark', paper_bgcolor='#0b0e11', plot_bgcolor='#0b0e11', height=350, margin=dict(l=0,r=0,t=0,b=0), xaxis=dict(showgrid=False), yaxis=dict(gridcolor='#2d3748'))
        st.plotly_chart(fig, use_container_width=True)

st.markdown("<hr style='border-color:#2d3748;'>", unsafe_allow_html=True)
c_log1, c_log2 = st.columns([3, 1])
with c_log1: st.markdown("### 📓 LIVE TRADE BOOK")
with c_log2: 
    if st.button("🔄 Sync WebSocket Price"): st.rerun()

if len(st.session_state.trade_history) > 0:
    history_df = pd.DataFrame(st.session_state.trade_history)
    st.dataframe(history_df.style.applymap(lambda v: 'color: #00ff66;' if v > 0 else 'color: #ff3333;' if v < 0 else '', subset=['PnL (Points)']), use_container_width=True, hide_index=True)
else:
