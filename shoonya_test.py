...
# ==============================================================================
st.set_page_config(page_title="Scalper Pro AI v15.2", layout="wide", initial_sidebar_state="collapsed")
...
def save_trade(trade_data, is_nifty=False):
    ...
    if not os.path.exists(filename): 
        ...
        try:
            ...
            if 'Result' not in existing.columns:
                df_new.to_csv(filename, index=False)
            else:
...
# BUG FIX: Proper CSS styling functions for Pandas Dataframe
def style_results(val):
    val_str = str(val).upper()
    if 'TARGET' in val_str or 'PROFIT' in val_str:
        return 'background-color: rgba(0, 255, 102, 0.1); color: #00ff66; font-weight: bold;'
    elif 'SL HIT' in val_str or 'LOSS' in val_str or 'SQUARE-OFF' in val_str:
...
def style_swing_action(val):
    ...
    if 'STRONG BUY' in val_str:
        return 'background-color: rgba(0, 255, 102, 0.1); color: #00ff66; font-weight: bold;'
    elif 'POTENTIAL' in val_str:
        return 'background-color: rgba(255, 170, 0, 0.1); color: #ffaa00; font-weight: bold;'
    return ''
...
with col_h1: st.markdown("<h1 style='margin:0; font-weight:800; color:#e3e9f0;'>QUANT<span style='color:#deff9a;'>SCALPER AI</span> v15.2</h1>", unsafe_allow_html=True)
with col_h2:
...
with tab1:
    try:
        ...
        if not data.empty:
            ...
            with col_met2:
                # BUG FIX: Ensure only ONE execution card is rendered
                if active_trade is not None and not is_eod_ui:
                    ...
                    <div class='ex-card'>
                        ...
                        <div style='color:#8b949e; font-size:12px; margin-top:5px;'>Risk/Reward: 1:{abs(rrr)}</div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            if not n_hist.empty: 
                # Applying the correct bug-free styling function
                st.dataframe(n_hist[['Time (IST)','Action','Spot Entry','Spot Exit','Points','Result']].style.apply(lambda x: [style_results(val) if x.name == 'Result' else '' for val in x], axis=0), use_container_width=True, hide_index=True)
...
with tab3:
    ...
    if swing_results:
        ...
        # Applying the correct bug-free styling function for Swing Tab
        st.dataframe(df_swing.style.apply(lambda x: [style_swing_action(val) if x.name == 'Action' else '' for val in x], axis=0), use_container_width=True, hide_index=True)
...
with tab4:
    ...
    if not all_trades.empty and 'Result' in all_trades.columns:
        ...
        with col_an_ch1:
            ...
            fig_pts.update_layout(template='plotly_dark', paper_bgcolor='#0b0e11', plot_bgcolor='#0b0e11', height=300, margin=dict(l=0,r=0,t=10,b=0))
